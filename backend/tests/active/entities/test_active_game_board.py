...
def test_take_resources_moves_amount_between_players(
    two_player_game: entities.ActiveGame,
) -> None:
...
def test_monopoly_of_resource_takes_all_from_other_players(
    ...
    two_player_game.players["player-a"].resources[entities.ResourceCard.GOLD] = 1
    two_player_game.players["player-b"].resources[entities.ResourceCard.GOLD] = 4
    two_player_game.players["player-b"].resources[entities.ResourceCard.WOOD] = 2

    two_player_game.monopoly_of_resource(entities.ResourceCard.GOLD)

    assert (
        two_player_game.players["player-a"].resources[entities.ResourceCard.GOLD] == 5
    )
    assert (
        two_player_game.players["player-b"].resources[entities.ResourceCard.GOLD] == 0
    )
    assert (
        two_player_game.players["player-b"].resources[entities.ResourceCard.WOOD] == 2
    )
...
@pytest.fixture
def two_player_game() -> entities.ActiveGame:
    mountains = entities.Hex(q=0, r=0, type=entities.HexType.MOUNTAINS, number=1)
    return entities.ActiveGame(
        map=(mountains,),
        conquistator_location=entities.HexLocation(q=0, r=0),
