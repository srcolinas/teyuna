...
def test_declare_winner_when_at_least_ten(game: entities.ActiveGame) -> None:
    by = game.active_player
    game.players[by].played_cards[entities.WisdomCard.LEGACY_OF_THE_ELDERS] = 10

    assert actions.declare_winner_if_eligible(game, by) is True
    assert game.winner == by


def test_declare_winner_skips_when_below_ten(game: entities.ActiveGame) -> None:
    by = game.active_player
    game.players[by].played_cards[entities.WisdomCard.LEGACY_OF_THE_ELDERS] = 9

    assert actions.declare_winner_if_eligible(game, by) is False
