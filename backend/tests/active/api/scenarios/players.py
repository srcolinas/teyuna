...
class BasePlayer:
    ...
    def take_action(
        self, phase: actions.GamePhaseName, state: entities.ActiveGame
    ) -> None:
...
class GreedyBuilder(BasePlayer):
    ...
    def take_action(
        ...
        match phase:
            case actions.GamePhaseName.DICE_ROLL:
                rounds.advance_phase(self._client, self._game_id, self._token)
            case actions.GamePhaseName.TRADE_AND_BUILD:
    ...
    def _try_build_terrace(self, game: entities.ActiveGame) -> bool:
    ...
    def _try_build_path(self, game: entities.ActiveGame) -> bool:
    ...
    def _post_settlement(
        ...
        response = self._client.post(
            f"/active-games/{self._game_id}/settlements",
            json={
                "item": item.value,
                "location": {
    ...
    def _post_path(self, location: entities.Coordinate) -> tuple[bool, str]:
        self._client.cookies["session-token"] = self._token
        response = self._client.post(
            f"/active-games/{self._game_id}/paths",
            json={
                "location": {
                    "hex_coord": {"q": location.q, "r": location.r},
                    "direction": location.d,
