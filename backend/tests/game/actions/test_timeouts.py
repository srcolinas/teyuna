...
def test_set_timeout_and_timeout_for() -> None:
    ...
    try:
        registry.timeout_for(entities.GamePhaseName.DICE_ROLL)
    ...
    timeout = registry.timeout_for(entities.GamePhaseName.DICE_ROLL)
...
def test_timeout_first_placement_executes() -> None:
    ...
    assert result.succeeded is True
    assert result.error is None
    assert result.phase is entities.GamePhaseName.FIRST_PLACEMENT
...
def test_timeout_play_mamo_and_blessed(game: entities.Game) -> None:
    ...
    assert mamo_result.phase is entities.GamePhaseName.DICE_ROLL
    ...
    assert blessed_result.phase is entities.GamePhaseName.DICE_ROLL
...
def test_timeout_play_pathfinder_allows_empty(game: entities.Game) -> None:
    ...
    assert action.paths == ()
    game.phase = entities.GamePhaseName.DICE_PLAY_PATHFINDER
    result = cast(
...
async def test_apply_timeout_if_due_is_noop_when_not_expired() -> None:
    ...
    assert repository.retrieve(game_id).phase is entities.GamePhaseName.DICE_ROLL
...
async def test_second_timeout_is_noop_after_deadline_refresh() -> None:
    ...
    assert first.due_to_timeout is True
...
async def test_apply_timeout_if_due_is_noop_when_deadline_is_none() -> None:
    ...
    assert result is None
    assert event_broker._next_id[game_id] == 0
    assert repository.retrieve(game_id).phase is entities.GamePhaseName.END_GAME
