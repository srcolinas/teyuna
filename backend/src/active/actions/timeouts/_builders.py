...
from ..handlers import _first_placement, _discard_resources, _move_conquistator
from ..handlers import _placement, _play_blessed, _play_mamo, _play_pathfinder


def timeout_dice_roll(
    game: entities.ActiveGame, rng: random.Random
) -> _registry.PlayerAction:
    return _registry.PlayerAction(by=game.active_player, rng_=rng)
...
def timeout_discard_resources(
    ...
    return _discard_resources.DiscardResourcesAction(by=nick, count=count, rng_=rng)
...
def timeout_play_mamo(
    ...
    return _play_mamo.PlayMamoAction(by=game.active_player, resource=resource, rng_=rng)
...
def timeout_play_pathfinder(
    ...
    return _play_pathfinder.PlayPathfinderAction(
        by=game.active_player, paths=tuple(chosen), rng_=rng
    )
