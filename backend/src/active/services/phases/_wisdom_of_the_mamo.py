...
class WisdomOfTheMamoPhase(
    ...
    def run(
        ...
        match request.action:
            case _core.ClaimResourceAction(resource=resource):
                taken = actions.claim_resource_monopoly(
                    game, request.by, resource=resource
                )
                self._performed = True
                self._taken = taken
                return _core.RunOutcome(finished=True, value=taken)
            case _core.AdvancePhaseAction():
                taken = actions.claim_resource_monopoly_randomly(
    ...
    def on_exit(
        ...
        if not self._performed:
            self._taken = actions.claim_resource_monopoly_randomly(
                game, game.active_player, rnd=self._rnd
            )
        if game.mamo_return_phase is None:
            raise RuntimeError("mamo_return_phase must be set before exiting")
        next_phase = _core.GamePhaseName(game.mamo_return_phase)
