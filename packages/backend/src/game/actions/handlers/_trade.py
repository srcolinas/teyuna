...
def handle_propose_trade(
    game: entities.Game, action: teyuna_shared.ProposeTradeAction
) -> teyuna_shared.ProposeTradeResult:
...
def handle_accept_trade(
    game: entities.Game, action: teyuna_shared.AcceptTradeAction
) -> teyuna_shared.AcceptedTradeResult:
    previous_phase = game.phase
    proposal = game.trade_proposals.get(action.id)
    if proposal is None:
        return teyuna_shared.AcceptedTradeResult(
            previous_phase=previous_phase,
            next_phase=game.phase,
            action=action,
...
def handle_trade_with_supply(
    game: entities.Game, action: teyuna_shared.TradeWithSupplyAction
) -> teyuna_shared.TradedWithSupplyResult:
    previous_phase = game.phase
    if game.active_player != action.by:
        return teyuna_shared.TradedWithSupplyResult(
            previous_phase=previous_phase,
            next_phase=game.phase,
            action=action,
...
def _trade_rate(
    ...
    for location, harbour_resource in teyuna_shared.HARBOUR_LOCATIONS.items():
        ...
        if harbour_resource is None:
            rate = min(rate, teyuna_shared.GENERIC_HARBOUR_TRADE_RATE)
        elif harbour_resource == offers:
            rate = min(rate, teyuna_shared.SPECIFIC_HARBOUR_TRADE_RATE)
    return rate
...
def _ensure_resources(
    ...
    cost: teyuna_shared.ResourceCount,
    *,
    reason: str,
) -> str | None:
