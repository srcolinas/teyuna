...
def test_raises_when_insufficient_resources_of_type(
    ...
    game.players[player].resources = collections.Counter(
        {
            teyuna_core.ResourceCard.WOOD: 6,
...
def test_discard_removes_player_and_stays_in_phase_when_others_remain(
    ...
    supply_before = game.resource_supply[teyuna_core.ResourceCard.WOOD]
    ...
    assert game.players[player].resources[teyuna_core.ResourceCard.WOOD] == 4
    assert game.resource_supply[teyuna_core.ResourceCard.WOOD] == supply_before + 4
