...
async def _discard(
    ...
    phase = await context.client.discard_resources(count)
    logger.info(
...
async def _dice_roll(
    ...
    if teyuna_shared.WisdomCard.WARRIOR in hand.wisdom_cards and _RNG.random() < 0.5:
        phase = await context.client.play_wisdom_card(teyuna_shared.WisdomCard.WARRIOR)
        logger.info("%s played warrior (next phase %s)", context.nickname, phase)
        return
...
async def _move_conquistator(
    ...
    await context.client.move_conquistator(location, take_from=take_from)
    logger.info(
...
async def _buy_wisdom(context: entities.PlayerContext, logger: logging.Logger) -> None:
    phase = await context.client.buy_wisdom_card()
    logger.info("%s bought wisdom card (next phase %s)", context.nickname, phase)
...
async def _trade_with_supply(
    ...
    phase = await context.client.trade_with_supply(offers=offers, requests=requests)
    logger.info(
